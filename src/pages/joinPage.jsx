...
const JoinPage = () => {
  ...
  const handlePasswordChange = (e) => {
    ...
    setPasswordStrength(passwordRegex.test(password) ? "보안성 : 강함" : "비밀번호는 영문, 숫자, 특수문자를 포함해 8자 이상이어야 합니다.");
  ...
  const handleSignup = async (e) => {
    ...
    const birth = `${formData.birthYear}-${String(formData.birthMonth).padStart(2, '0')}-${String(formData.birthDay).padStart(2, '0')}`;

    // formDataToSendWithBirth를 정의
    const { confirmPassword, userId, phoneNumber, updateDate, ...formDataToSend } = formData;
    const formDataToSendWithBirth = { ...formDataToSend, birth, user_id: userId, phone_number:phoneNumber, update_date: updateDate }; // 오늘 날짜 추가
    ...
    <div className="join-container">
      ...
      <form className="join-form" onSubmit={handleSignup}>
        ...
        <div className="birth-select">
          <select name="birthYear" value={formData.birthYear} onChange={handleDateChange}>
          ...
          <select name="birthMonth" value={formData.birthMonth} onChange={handleDateChange}>
          ...
          <select name="birthDay" value={formData.birthDay} onChange={handleDateChange}>
